...
CF = $(CFLAGS) $(CF_NOOPT)
LFLAGS1 =
LF = -g -o unzip$E $(LFLAGS1)
LF2 = -g

# UnZipSFX flags
SL = -g -o unzipsfx$E $(LFLAGS1)
SL2 = $(LF2)

# fUnZip flags
FL = -g -o funzip$E $(LFLAGS1)
FL2 = $(LF2)
...
generic_shlib:	unix_make
 ...
 'which is UnZip linked with the DLL).  This target is an example only.'
	@echo ""
	$(MAKE) objsdll CC=gcc CFLAGS="-g -O3 -Wall -fPIC -DDLL"
	gcc -shared -Wl,-soname,libunzip.so.0 -o libunzip.so.0.4 $(OBJSDLL)
	$(RM) libunzip.so.0 libunzip.so
	$(LN) -s libunzip.so.0.4 libunzip.so.0
	$(LN) -s libunzip.so.0 libunzip.so
	gcc -c -O unzipstb.c
	gcc -g -o unzip_shlib unzipstb.o -L. -lunzip
...
7300_gcc:	unix_make
	$(MAKE) unzips CC=gcc LD=gcc LF2="" CFLAGS="-g -O2" \
...
cygwin:		unix_make
	$(MAKE) unzips CC=gcc LD=gcc AS=gcc\
	 CFLAGS="-g -O3 -DASM_CRC -DNO_LCHOWN -DNO_LCHMOD"\
...
freebsd:		unix_make
	...
	$(MAKE) unzips CC=gcc LD=gcc AS=gcc\
	 CFLAGS="-g -O3 -Wall -DASM_CRC -DBSD"\
	 AF="-Di386 $(AF)" CRCA_O=crc_gcc$O
...
#
gcc:		unix_make
	$(MAKE) unzips CC=gcc LD=gcc CFLAGS="-g -O3" LF2=""
	$(STRIP) $(UNZIPS)
...
isc:		unix_make
	$(MAKE) unzips LF2="-lc_s $(LF2)" CRCA_O=crc_sysv$O \
	 CFLAGS="-g -O" LOC="-DASM_CRC -DSYSV -DNO_UID_GID -DNEED_PTEM -DNO_LCHOWN -DNO_LCHMOD $(LOC)" \
...
isc_gcc:	unix_make
	$(MAKE) unzips AS=gcc CC=gcc LD=gcc CRCA_O=crc_gcc$O \
	 ...
	 CFLAGS="-g -O3" LOC="-DSYSV -DASM_CRC -DNO_UID_GID -DNEED_PTEM -DNO_LCHOWN -DNO_LCHMOD $(LOC)" \
	 AF="-DNO_UNDERLINE -Djecxz=jcxz -DALIGNMENT='.align 16' $(AF)"
	$(STRIP) $(UNZIPS)
...
#
linux_lcc:	unix_make
	$(MAKE) unzips CC=lcc LD=lcc CFLAGS="-g -O3 -Wall -D__inline__= "
...
linux_shlib:	unix_make
	...
	gcc -g -shared -Wl,-soname,libunzip.so.0 -o libunzip.so.0.4 $(OBJSDLL)\
	 crc_gcc.pic.o
...
linux_shlibz:	unix_make
	$(MAKE) objsdll CC=gcc AS=gcc AF="-fPIC -Di386 $(AF)" CRCA_O=crc_gcc$O\
	 CFLAGS="-g -O3 -Wall -fPIC" LOC="-DDLL -DUSE_ZLIB -DASM_CRC $(LOC)"
	gcc -shared -Wl,-soname,libunzip.so.0 -o libunzip.so.0.4 $(OBJSDLL)\
	 crc_gcc.pic.o
	ln -sf libunzip.so.0.4 libunzip.so.0
	gcc -c -O unzipstb.c
	gcc -g -o unzip unzipstb.o -L. -lunzip -lz
...
macosx:	unix_make
	$(MAKE) unzips CFLAGS="-g -O3 -Wall -DBSD" LF2=""
	$(STRIP) $(UNZIPS)

# Macintosh MacOS X (Unix-compatible enviroment), using gcc
macosx_gcc:	unix_make
	$(MAKE) unzips CC=gcc CFLAGS="-g -O3 -Wall -DBSD" LF2=""
	$(STRIP) $(UNZIPS)
...
sco_dos:	unix_make
	$(MAKE) unzips CFLAGS="-g -O -dos -M0" M=msdos OSDEP_H="msdos/doscfg.h" \
...
sysv_gcc:	unix_make
	$(MAKE) unzips CC=gcc LD=gcc CFLAGS="-g -O2 -DSYSV" LF2=""
	$(STRIP) $(UNZIPS)
